...
const { date, data, todayEarnings } = JSON.parse(fs.readFileSync(dataTempPath));
  ...
  axios.get(`${baseUrl}/${item.code}.js?rt=${curTime}`)
    .then(res => {
      ...
      if (!item.isUpdate) {
        ...
        estimatePrice = parseInt((estimatePrice + curEstimatePrice) * 100) / 100;
        console.log(`${item.name}：${curEstimatePrice}`);
      }
        ...
        .end((err, res) => {
          if (err) {
            ...
            if (dateText.includes(date)) {
              ...
              // 计算净值
              realPrice = parseInt((realPrice + curRealPrice) * 100) / 100;
              if (!item.isUpdate) {
